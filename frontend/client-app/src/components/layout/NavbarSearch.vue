<template>
  <div class="relative group">
    ...
    <!-- Иконка поиска (слева) -->
    <div 
      class="absolute inset-y-0 left-0 flex items-center pl-4 transition-colors duration-300"
    ...
    <div 
      v-else-if="search.length >= 2 && !isLoading && searchAttempted && searchResults.length === 0"
    ...
    >
      <span class="flex items-center">
        {{ $t('navbar.searchHint') }} 
        <kbd class="ml-1 px-1.5 py-0.5 bg-gray-100 border border-gray-300 rounded text-gray-800 font-semibold text-xs">Enter</kbd>
      </span>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async fetchSearchResults(query) {
      try {
        ...
        const response = await productService.getProducts(0, query, null, null, null, 4);
