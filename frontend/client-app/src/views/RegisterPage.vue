    ...
    <div class="flex-grow flex items-center justify-center bg-gray-100">
      <div class="w-full max-w-md p-8 bg-white rounded-lg shadow-md">
        ...
        <form @submit.prevent="handleRegister">
          <ValidationInput id="firstName" :label="$t('register.firstName')" type="text" v-model="firstName"
            validationRules="required|name" :error-messages="{
              required: $t('validation.name.required'),
              name: $t('validation.name.pattern')
            }" ref="firstNameInput" @validateInput="updateValidationState('firstName', $event)" />

          <ValidationInput id="lastName" :label="$t('register.lastName')" type="text" v-model="lastName"
            validationRules="required|name" :error-messages="{
              required: $t('validation.lastName.required'),
              name: $t('validation.lastName.pattern')
            }" ref="lastNameInput" @validateInput="updateValidationState('lastName', $event)" />

          <ValidationInput id="email" :label="$t('register.email')" type="email" v-model="email"
            validationRules="required|email" ref="emailInput" @validateInput="updateValidationState('email', $event)" />

          <ValidationInput id="phone" :label="$t('register.phone')" type="tel" v-model="phone"
            validationRules="required|phone" :error-messages="{
              required: $t('validation.phone.required'),
              phone: $t('validation.phone.pattern')
            }" ref="phoneInput" @validateInput="updateValidationState('phone', $event)" />

          <ValidationInput id="password" :label="$t('register.password')" type="password" v-model="password"
          ...
          <ValidationInput id="confirmPassword" :label="$t('register.confirmPassword')" type="password"
            ...
            }" :compareWith="password" ref="confirmPasswordInput"
